...
A missing file is valid: `loadRawConfig` returns `{}` and `mergeWithDefaults` applies package defaults, including gate model `anthropic/claude-haiku-4-5`. Source `pi-review.json` overrides that: gate and bundled reviewers use `xai/grok-4.6` at thinking `low`. `conventions` stays disabled. Routing stays `adaptive`.
...
| Artifact         | Class                                       | Why                                                        |
| ---------------- | ------------------------------------------- | ---------------------------------------------------------- |
| `pi-review.json` | `copy` (`followsSymlinks: false`, hardlink) | No secrets; atomic rename; package would replace a symlink |
