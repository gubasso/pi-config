...
Verified against installed 0.68.0 and upstream
https://github.com/nicobailon/pi-subagents/blob/main/docs/configuration.md.
...
package writes the file when `saveConfig` / `updateConfig` runs.
...
Optional custom tool prose: `subagent-tool-description.md` at the
agent-dir root (project config dir first, then
`~/.pi/agent/subagent-tool-description.md`). The package does not
write it.
...
Source `extensions/subagent/config.json` is `{}` until this
operator sets policy. Do not author `subagent-tool-description.md`
until custom tool prose is wanted. After install, `/reload` or
start a new session.
