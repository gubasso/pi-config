...
in this repo. The install tree does not. Every plugin config file
is classified and landed from this repo.
...
run `pi install` and does not copy `npm/`. It does land classified
sidecars.
...
Then read upstream docs and list every config file the package
reads under the live agent dir. Classify each one before `pi
...
Write `docs/plugins/<plugin-name>/README.md`, `SPEC.md`, and
`sidecars.json`. Create the source file for every required
...
Commit `settings.json` and the plugin docs when the pin is a
decision you want on every host. Leave `npm/` untracked.
...
Create live-only secret files on that host if needed. Do not copy
them from another machine unless you intend to.
...
tree. Delete `docs/plugins/<plugin-name>/` and any source sidecars
that existed only for that pin. Commit the JSON and docs change.
